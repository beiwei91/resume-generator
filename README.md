...
写 Markdown，实时看 A4 排版，一键导出 PDF。**零依赖、纯本地**：不用装、不联网、不上传，一份 `.md` 就是你的简历。
...
界面左边写 Markdown，右边就是 A4 预览。首次打开会载入示例简历，直接改就行。

## 界面功能

**工具栏**：文档名（也是导出 PDF 的文件名）、已保存文档、新建 / 另存为 / 删除、导入 MD、下载 MD、保存到磁盘、语法速查、导出 PDF。

**编辑区按钮**：加粗、斜体、行内代码、章节、条目、要点、表格、分页、分隔线、**证件照**。

**预览区右上**：模板、主色、「排版」面板（字体 / 字号 / 行距 / 页边距 / 缩放 / 分页参考线，有证件照时会多出宽度调节与「移除证件照」）。
...
| `Ctrl + P` | 导出 PDF |
| `Ctrl + B` | 加粗选中文本 |
| `Tab` | 插入两个空格（在列表里就是二级要点） |
| `Enter` | 列表项自动续写下一个标记；空条目上按 Enter 退出列表 |
| 拖 `.md` 到编辑区 | 导入该文件 |
...
```powershell
npm test            # 渲染引擎 31 项断言：解析、行内语法、转义、分页、模板
npm run verify      # 导出链路：真的调 Chrome 出 PDF，校验 A4 尺寸、页边距、分页
npm run verify:app  # 双击即用链路：图标、启动器命令与 URL 编码、快捷方式、单文件版、无服务残留
```

## 目录结构

```
index.html                  工作台界面（编辑器 + 预览 + 导出）
